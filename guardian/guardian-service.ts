...
import {
    ...
    verifyCreatorKey,
    verifyPin,
} from "./creatorKeyManager.js";
import { authorize, listPolicies } from "./guardian.js";
...
const GUARDIAN_SECRET =
  process.env.GUARDIAN_SECRET || "GUARDIAN_SOVEREIGN_2026";
...
if (!isVaultReady()) {
  ...
  if (pin && key) {
    ...
    console.warn(
      "[Guardian] Vault not ready. Set CREATOR_PIN and CREATOR_KEY env vars to initialize.",
    );
  }
}
...
const server = createServer(async (req, res) => {
  ...
  res.setHeader(
    ...
    "Content-Type, X-Guardian-Secret, X-Creator-Session",
  );
  ...
  try {
    ...
    if (method === "GET" && url === "/policies") {
      if (!checkGuardianAuth(req))
        return respond(res, 403, { error: "GUARDIAN_AUTH_REQUIRED" });
    ...
    if (method === "POST" && url === "/authorize") {
      ...
      const result = authorize(action, payload as Record<string, unknown>, {
        creatorSignature,
      });
...
server.listen(PORT, "127.0.0.1", () => {
  ...
  console.log(
    `║  Vault: ${isVaultReady() ? "✅ READY" : "⚠️  NOT INITIALIZED"}           ║`,
  );
  console.log(`╚══════════════════════════════════════╝\n`);
});
