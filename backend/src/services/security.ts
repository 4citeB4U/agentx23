...
const SESSION_TIMEOUT_MS = parseInt(process.env.SESSION_TIMEOUT_MS || '1800000'); // Default 30 minutes
...
export const securityMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const isScreenshotRoute = normalizedPath.includes('/device/screenshot') || normalizedOriginalUrl.includes('/api/device/screenshot');
    const isActRoute = normalizedPath.includes('/device/act') || normalizedOriginalUrl.includes('/api/device/act');
    const isDeploymentInitiate = normalizedPath.includes('/deployment/initiate') || normalizedOriginalUrl.includes('/api/deployment/initiate');
    const isFsRoute = normalizedPath.includes('/fs/') || normalizedPath === '/fs' || normalizedOriginalUrl.includes('/api/fs/');
    const isRuntimeRoute = normalizedPath.includes('/services/runtime') || normalizedOriginalUrl.includes('/api/services/runtime');
    const isChatRoute = normalizedPath.includes('/chat') || normalizedOriginalUrl.includes('/api/chat');
    const isMcpRoute = normalizedPath.includes('/mcp') || normalizedOriginalUrl.includes('/api/mcp');
    const isTunnelRoute = normalizedPath.includes('/tunnel') || normalizedOriginalUrl.includes('/api/tunnel');
    const isAgentsRoute = normalizedPath.includes('/agents') || normalizedOriginalUrl.includes('/api/agents');
    const isBrainRoute  = normalizedPath.includes('/brain')  || normalizedOriginalUrl.includes('/api/brain');
    const isPhoneRoute  = normalizedPath.includes('/phone')  || normalizedOriginalUrl.includes('/api/phone');
    const isAppsRoute   = normalizedPath.includes('/apps')   || normalizedOriginalUrl.includes('/api/apps');
    const isSystemStatusRoute = normalizedPath === '/services/system-status' || normalizedOriginalUrl === '/api/services/system-status';

    const handshakeHeader = (req.headers['x-neural-handshake'] as string | undefined)?.trim();
    const expectedHandshake = (process.env.NEURAL_HANDSHAKE || process.env.NEURAL_HANDSHAKE_KEY || '').trim();
    const hasValidHandshake = Boolean(expectedHandshake && handshakeHeader && handshakeHeader === expectedHandshake);
    ...
    if (isSystemStatusRoute) {
        if (!hasValidHandshake) {
            return res.status(401).json({ error: 'MISSING_HANDSHAKE', message: 'x-neural-handshake required.' });
        }
        return next();
    }
    ...
    if (hasValidHandshake && (remotePublicRead || remotePublicWrite || remotePublicDeploy || remoteFsAllowed || remoteRuntimeAllowed || remoteChatAllowed || remoteMcpAllowed || remoteTunnelAllowed || remoteAgentsAllowed || remoteBrainAllowed || remotePhoneAllowed || remoteAppsAllowed || remoteStatusAllowed)) {
        ...
        let limit = 60;
        let windowMs = 60_000;
        if (remotePublicRead) { limit = 20; windowMs = 5_000; }          // screenshots
        else if (remotePublicWrite) { limit = 60; windowMs = 10_000; }   // input actions
        else if (remoteChatAllowed) { limit = 12; windowMs = 60_000; }   // chat requests
        else if (remoteFsAllowed) { limit = 30; windowMs = 60_000; }     // fs operations
        else if (remoteMcpAllowed) { limit = 10; windowMs = 60_000; }    // start/stop/status/logs
    ...
    if (governorStatus.isLocked && isWriteRequest) {
        return res.status(403).json({
            ...
            message: 'System is in read-only Safe Mode. Write operations are disabled.'
        });
    }
    ...
    const quotaCheck = ResourceGovernor.checkQuota(quotaType, deviceId || 'unknown');
    ...
    if (!deviceId || !signature || !timestamp || !nonce) {
        return res.status(401).json({
            ...
            message: 'Every request must be cryptographically signed by an approved device node.'
        });
    }
    ...
    const isValid = CryptoUtils.verifySignature(message, signature, device.secret);
