...
const BRAIN_URL   = `http://127.0.0.1:${process.env.NEURAL_ROUTER_PORT || 8004}`;
...
export type ExecutionMode = 'auto' | 'single_pass' | 'multi_pass' | 'qwen' | 'gemini';

export interface StructuredOutput {
    intent:       string;
    plan:         string[];
    tool_calls:   { tool: string; args: Record<string, unknown>; verified?: boolean }[];
    analysis:     string;
    final_answer: string;
    confidence:   number;
}
...
const CODE_KW = ['typescript','python','api','route','schema','function','test','debug',
                 'endpoint','backend','scaffold','refactor','sql','query'];
const UI_KW   = ['spline','puppeteer','component','react','frontend','css','tailwind',
                 'animate','3d','scene','stitch','browser','webgl','design'];
const CDL_KW  = ['cdl','logistics','driver','fleet','manifest','shipment','cargo','warehouse'];
...
const COMPLEX_KW = ['build','create an app','architect','design system','deploy',
                    'orchestrate','migrate','integrate','scaffold entire','full stack',
                    'pipeline','workflow','automate','refactor all','multi-step'];
 ...
 */
export async function routeToBrain(opts: BrainRouterOptions): Promise<BrainResponse> {
 ...
 */
export async function teachBrain(prompt: string, domain: DomainName = 'auto'): Promise<unknown> {
