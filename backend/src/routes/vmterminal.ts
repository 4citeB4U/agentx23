...
import { Client as SSHClient, type ConnectConfig, type Channel } from 'ssh2';
...
vmterminalRouter.post('/session', (req, res) => {
  ...
  const session: VmSession = {
    id: sessionId, ssh, channel: null, mode,
    created: new Date().toISOString(), alive: false, outputBuf: '',
  };
  ...
  ssh.on('error', (err) => {
    ...
    if (!res.headersSent) {
      res.status(502).json({ error: 'SSH_CONNECT_FAILED', detail: err.message });
    }
  });
...
vmterminalRouter.get('/status', (req, res) => {
  ...
  const list = [...sessions.entries()].map(([id, s]) => ({
    id, mode: s.mode, alive: s.alive,
    connected: !!s.ws, created: s.created,
  }));
  res.json({
    vmHost: VM_HOST,
    vmPort: VM_PORT,
...
vmterminalWss.on('connection', (ws: WebSocket, req: Request) => {
  ...
  if (!session || !session.alive) {
    ws.send(JSON.stringify({ type: 'error', msg: 'SESSION_NOT_FOUND_OR_DEAD' }));
    ws.close();
    return;
  }
  ...
  session.ssh.shell({ term: 'xterm-256color', cols, rows }, (err, channel) => {
    if (err) {
    ...
    ws.on('message', async (raw: Buffer | string) => {
      ...
      if (msg.type === 'input') {
        ...
        channel.setWindow(msg.rows || 30, msg.cols || 120, 0, 0);
      } else if (msg.type === 'set_mode') {
        if (['safe', 'build', 'admin'].includes(msg.mode)) {
          session.mode = msg.mode as PolicyMode;
          ws.send(JSON.stringify({ type: 'mode_changed', mode: session.mode }));
        }
      }
    });
  });
