...
deviceRouter.get('/screenshot', async (req, res) => {
    try {
        ...
        const useDisplayIndex = Number.isFinite(displayIndexRaw) && displayIndexRaw >= 1;

        // Desktop agent /screen is single-display; if a display is requested, use screenshot-desktop.
        if (!useDisplayIndex) {
            const desktopResponse = await fetch(`http://127.0.0.1:${DESKTOP_AGENT_PORT}/screen`);
        ...
        if (useDisplayIndex) {
            ...
            if (!selectedDisplay) {
                return res.status(404).json({ error: 'DISPLAY_NOT_FOUND', available: displays.length, requested: Math.floor(displayIndexRaw) });
            }
        ...
        res.status(500).json({ error: 'SCREEN_CAPTURE_FAILED', details: String(error) });
    }
});
/**
 * POST /api/device/act
    ...
    try {
        ...
        try {
            ...
            await AuditLogger.log({
                ...
                meta: {
                    ip,
                    action,
                    hasCoordinates: Boolean(Array.isArray(coordinates) && coordinates.length >= 2),
        ...
        if (contentType.includes('application/json')) {
            try {
                ...
                return res.status(502).json({ error: 'DESKTOP_AGENT_BAD_JSON', status: response.status, raw: raw.slice(0, 2000) });
            }
        }
        return res.status(502).json({ error: 'DESKTOP_AGENT_NON_JSON', status: response.status, raw: raw.slice(0, 2000) });
    } catch (error: any) {
        console.error('[device] Action failed:', error);
        res.status(500).json({ error: 'ACTION_EXECUTION_FAILED', details: String(error) });
    }
});
