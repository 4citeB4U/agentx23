...
async function audit(req: any, entry: Record<string, unknown>) {
    ...
    const line = JSON.stringify({ ts: nowIso(), ip: req.ip, method: req.method, url: req.originalUrl, ...entry }) + '\n';
    await fsp.appendFile(AUDIT_PATH, line, 'utf8');
}

function loadConfig(): FsAliasConfig {
    // Highest priority: env JSON override
    const envJson = (process.env.FS_ALIASES_JSON || process.env.FS_ALIAS_JSON || '').trim();
    if (envJson) {
        try {
    ...
    return {
        ...
        blockedPatterns: ['.env', '.ssh', '.git', 'System32', 'Windows', 'Program Files', 'node_modules']
    };
}
...
function safeRelPath(input: unknown): string {
    ...
    if (trimmed.startsWith('\\\\') || trimmed.startsWith('//')) throw new Error('UNC_PATH_FORBIDDEN');
    if (trimmed.startsWith('/') || trimmed.startsWith('\\')) throw new Error('ABSOLUTE_PATH_FORBIDDEN');

    const normalized = path.win32.normalize(trimmed.replace(/\//g, '\\')).replace(/^\\+/, '');
    if (normalized === '..' || normalized.startsWith('..\\')) throw new Error('PATH_TRAVERSAL');
    return normalized;
}

function resolveWithin(root: string, rel: string): { rootResolved: string; fullPath: string } {
    ...
    const ok = fullPath.toLowerCase() === rootResolved.toLowerCase() || fullPath.toLowerCase().startsWith(rootPrefix.toLowerCase());
...
function resolveRootAndPath(rootAlias: string, rawPathInput: unknown, cfg: FsAliasConfig): { root: string; rootAlias: string; rel: string } {
    const rawStr = String(rawPathInput ?? '').replace(/\//g, '\\').trim();
    ...
    if (rootAlias && cfg.aliases[rootAlias]) {
        return { root: getAliasOrThrow(rootAlias, cfg), rootAlias, rel: safeRelPath(rawPathInput) };
    }
    ...
    if (isAbsolute) {
        ...
        for (const [id, aliasRoot] of sortedAliases) {
            ...
            const resolvedPrefix = resolved.endsWith('\\') ? resolved : resolved + '\\';
            if (rawStr.toLowerCase().startsWith(resolvedPrefix.toLowerCase()) || rawStr.toLowerCase() === resolved.toLowerCase()) {
...
fsRouter.get('/list', async (req, res) => {
    ...
    try {
        ...
        const { root, rootAlias, rel } = resolveRootAndPath(rawAlias, req.query.path, cfg);
        ...
        await audit(req, { action: 'FS_LIST', status: 'SUCCESS', root: rootAlias, path: rel.replace(/\\/g, '/') });
        res.json({ root: rootAlias, path: rel.replace(/\\/g, '/'), entries: mapped });
    } catch (e: any) {
        await audit(req, { action: 'FS_LIST', status: 'ERROR', error: String(e?.message || e) });
...
fsRouter.get('/read', async (req, res) => {
    ...
    try {
        ...
        await audit(req, { action: 'FS_READ', status: 'SUCCESS', root: rootAlias, path: rel.replace(/\\/g, '/'), sizeBytes: stat.size });
        fs.createReadStream(fullPath).pipe(res);
    } catch (e: any) {
        await audit(req, { action: 'FS_READ', status: 'ERROR', error: String(e?.message || e) });
        res.status(400).json({ error: String(e?.message || e) });
    }
});
...
fsRouter.post('/write', async (req, res) => {
    ...
    try {
        ...
        await audit(req, { action: 'FS_WRITE', status: 'SUCCESS', root: rootAlias, path: rel.replace(/\\/g, '/') });
        res.json({ success: true });
    } catch (e: any) {
        await audit(req, { action: 'FS_WRITE', status: 'ERROR', error: String(e?.message || e) });
        res.status(400).json({ error: String(e?.message || e) });
    }
});
...
fsRouter.post('/upload', upload.single('file'), async (req, res) => {
    ...
    const blocked = getBlockedPatterns(cfg);
    try {
        if (!req.file) throw new Error('NO_FILE_UPLOADED');
        const rootAlias = String(req.body?.root || '').trim();
        const rel = safeRelPath(req.body?.path);
        const root = getAliasOrThrow(rootAlias, cfg);
        const { fullPath } = resolveWithin(root, rel);
        ...
        await audit(req, { action: 'FS_UPLOAD', status: 'SUCCESS', root: rootAlias, path: rel.replace(/\\/g, '/'), sizeBytes: req.file.size });
        res.json({ success: true, sizeBytes: req.file.size });
    } catch (e: any) {
        try {
        ...
        await audit(req, { action: 'FS_UPLOAD', status: 'ERROR', error: String(e?.message || e) });
        res.status(400).json({ error: String(e?.message || e) });
    }
});
...
fsRouter.post('/mkdir', async (req, res) => {
    ...
    try {
        ...
        await audit(req, { action: 'FS_MKDIR', status: 'SUCCESS', root: rootAlias, path: rel.replace(/\\/g, '/') });
        res.json({ success: true });
    } catch (e: any) {
        await audit(req, { action: 'FS_MKDIR', status: 'ERROR', error: String(e?.message || e) });
        res.status(400).json({ error: String(e?.message || e) });
    }
});
...
fsRouter.post('/move', async (req, res) => {
    ...
    try {
        ...
        if (isBlocked(src, blocked) || isBlocked(dst, blocked)) throw new Error('ACCESS_DENIED_BLOCKED');

        await fsp.mkdir(path.win32.dirname(dst), { recursive: true });
        await fsp.rename(src, dst);
        await audit(req, { action: 'FS_MOVE', status: 'SUCCESS', root: rootAlias, src: srcRel.replace(/\\/g, '/'), dst: dstRel.replace(/\\/g, '/') });
        res.json({ success: true });
    } catch (e: any) {
        await audit(req, { action: 'FS_MOVE', status: 'ERROR', error: String(e?.message || e) });
        res.status(400).json({ error: String(e?.message || e) });
    }
});
...
fsRouter.post('/copy', async (req, res) => {
    ...
    try {
        ...
        await audit(req, { action: 'FS_COPY', status: 'SUCCESS', root: rootAlias, src: srcRel.replace(/\\/g, '/'), dst: dstRel.replace(/\\/g, '/') });
        res.json({ success: true });
    } catch (e: any) {
        await audit(req, { action: 'FS_COPY', status: 'ERROR', error: String(e?.message || e) });
        res.status(400).json({ error: String(e?.message || e) });
    }
});
...
fsRouter.delete('/delete', async (req, res) => {
    ...
    try {
        ...
        await audit(req, { action: 'FS_DELETE', status: 'SUCCESS', root: rootAlias, path: rel.replace(/\\/g, '/') });
        res.json({ success: true });
    } catch (e: any) {
        await audit(req, { action: 'FS_DELETE', status: 'ERROR', error: String(e?.message || e) });
        res.status(400).json({ error: String(e?.message || e) });
    }
});
