...
const NEURAL_ROUTER_PORT = Number(process.env.NEURAL_ROUTER_PORT || 8004);
...
const sendTelegramVoice = async (text: string) => {
    ...
    try {
        ...
        if (!ttsRes.ok) {
            console.warn('[telegram-voice] TTS responded', ttsRes.status, '— skipping voice note');
            return;
        }
...
chatRouter.get('/history', async (req, res) => {
    try {
        ...
        const messages = messageRouter.getHistory({
            ...
            source: source === 'web' || source === 'voice' || source === 'telegram' || source === 'system' ? source : undefined
        });
...
chatRouter.post('/', async (req, res) => {
    try {
        ...
        if ((source || 'web') !== 'telegram') {
            sendTelegramMessage(`📨 ${source === 'voice' ? '🎤 Voice' : '💬 User'}: ${msgText}`).catch((err: any) => {
        ...
        if (planMatch) {
            planSteps = planMatch[1].split('|').map((s: string) => s.trim()).filter(Boolean);
            responseText = responseText.replace(/BUILD_PLAN::([\s\S]+?)::END_PLAN\n?/, '').trim();
        }

        // Send Agent Lee's response to Telegram after AI finishes
        if ((source || 'web') !== 'telegram') {
            sendTelegramMessage(`🤖 Agent Lee: ${responseText}`).catch((telegramError: any) => {
                console.error('[telegram] Outbound relay error:', telegramError?.message || telegramError);
            });
        }

        res.json({ ...response, text: responseText, ...(planSteps ? { plan: planSteps } : {}) });
    } catch (error: any) {
        res.status(500).json({ error: error.message });
    }
});
...
chatRouter.post('/tts', async (req, res) => {
    try {
        ...
        const response = await fetch(`http://localhost:${NEURAL_ROUTER_PORT}/tts`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                text,
 ...
 */
const TELEGRAM_POLL_INTERVAL = Number(process.env.TELEGRAM_POLL_INTERVAL || 3500);

export function startTelegramPolling() {
    const { token, userId } = getTelegramConfig();
    if (!token || !userId) {
        console.log('[telegram-poll] Bot token or user ID not configured — polling disabled');
        return;
    }
    console.log(`[telegram-poll] 📡 Polling Telegram every ${TELEGRAM_POLL_INTERVAL}ms`);
    setInterval(async () => {
        try {
