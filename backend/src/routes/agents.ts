...
import { routeToBrain, classifyDomain, needsMultiPass } from '../services/BrainRouter.js';
...
async function governedDispatch<T>(
    ...
    }).catch((e: Error) => console.warn(`[governance] brain log failed for ${agentName}:`, e.message));
...
agentsRouter.get('/status', (_req, res) => {
    res.json({
        ...
        agents: [
            {
                name:         'insforge',
                description:  'Backend code-generation sub-agent (APIs, routes, schemas, services)',
                tasks:        ['scaffold_api', 'generate_route', 'generate_schema', 'generate_service', 'review_code', 'custom'],
                endpoint:     '/api/agents/insforge',
            },
            {
                name:         'stitch',
                description:  'Frontend / UI design sub-agent (React, TypeScript, Tailwind)',
                tasks:        ['scaffold_component', 'generate_page', 'generate_styles', 'generate_hook', 'design_system', 'preview_url', 'custom'],
            ...
            },
            {
                ...
                tasks:        ['embed_scene', 'generate_animation', 'scaffold_3d_page', 'design_brief', 'orb_component', 'status'],
                endpoint:     '/api/agents/spline',
                team_invite:  'https://app.spline.design/team-invitation/e6646575-c700-467b-8304-579fdebc4c28',
            },
        ],
    });
});
...
agentsRouter.post('/insforge', async (req, res) => {
    ...
    if (!task || !input) {
        return res.status(400).json({ ok: false, error: 'task and input are required' });
    }
