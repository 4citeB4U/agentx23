...
export const ParticleBanner: React.FC<ParticleBannerProps> = ({ messages = [], height = 40 }) => {
    ...
    useEffect(() => {
        ...
        const createGroup = (index: number): MessageGroup => {
            ...
            return { particles, width: msgWidth, x: canvas.width + 50, spawnedNext: false, text };
        };
        ...
        const animate = () => {
            ...
            for (let i = activeGroups.length - 1; i >= 0; i--) {
                ...
                if (!group.spawnedNext && (group.x + group.width) < (canvas.width - spawnPadding)) {
    ...
    return (
        <div
            ref={containerRef}
            className="w-full relative overflow-hidden bg-black/80 border-y border-white/5"
