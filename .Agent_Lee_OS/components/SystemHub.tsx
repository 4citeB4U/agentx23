import {
    Box, CheckCircle, Cpu, Database, Globe, Grid, Layers, Network, Radio, RotateCw,
    ShieldCheck, Smartphone, Sun, Timer, X, Zap
} from 'lucide-react';
...
const INVENTORY: ServiceNode[] = [
    { id: 'core-studio', name: 'Agent Lee Studio', role: 'Frontend Host', port: 3002, type: 'CORE', visualType: 'WAVE', status: 'ONLINE', icon: <Smartphone size={18} />, description: 'Main React application container and UI thread host.' },
    { id: 'core-orch', name: 'Orchestrator', role: 'System Logic', port: 3001, type: 'CORE', visualType: 'WAVE', status: 'ONLINE', icon: <Cpu size={18} />, description: 'Central decision engine routing intents to specific MCPs.' },
    { id: 'core-brain', name: 'Neural Brain', role: 'LLM Inference', port: 8001, type: 'CORE', visualType: 'GAUGE', status: 'ONLINE', icon: <Zap size={18} />, description: 'Local quantization engine running Llama-3-8b via Ollama.' },
    { id: 'core-mesh', name: 'Neural Mesh', role: 'WebSockets', port: 3003, type: 'CORE', visualType: 'WAVE', status: 'ONLINE', icon: <Network size={18} />, description: 'Real-time event bus handling duplex communication.' },
    { id: 'mcp-stitch', name: 'Stitch', role: 'The Architect', type: 'MCP', visualType: 'BARS', status: 'ONLINE', icon: <Layers size={18} />, description: 'Generative UI component builder and layout engine.' },
    { id: 'mcp-test', name: 'TestSprite', role: 'The Auditor', type: 'MCP', visualType: 'BARS', status: 'ONLINE', icon: <ShieldCheck size={18} />, description: 'Autonomous integration testing and DOM verification.' },
    { id: 'mcp-play', name: 'Playwright', role: 'The Ghost', type: 'MCP', visualType: 'BARS', status: 'ONLINE', icon: <Globe size={18} />, description: 'Headless browser automation for external web interaction.' },
    { id: 'mcp-ins', name: 'InsForge', role: 'The Scribe', type: 'MCP', visualType: 'BARS', status: 'ONLINE', icon: <Database size={18} />, description: 'Structured SQL/NoSQL database management layer.' },
    { id: 'wkr-desktop', name: 'AA Desktop', role: 'BG Worker', type: 'WORKER', visualType: 'WAVE', status: 'ONLINE', icon: <Box size={18} />, description: 'Background service worker for push notifications and sync.' },
];
...
const RadialGauge: React.FC<{ value: number, label: string, color: string }> = ({ value, label, color }) => {
    const radius = 36;
    const circumference = 2 * Math.PI * radius;
    const offset = circumference - (value / 100) * circumference;
    const glowClass = color === '#10b981'
    ...
    return (
        <div className="relative flex items-center justify-center w-full h-24">
            ...
            <div className="absolute flex flex-col items-center">
                <span className="text-xl font-bold text-white tracking-tighter">{value.toFixed(0)}%</span>
                <span className="text-[9px] text-gray-500 uppercase font-mono tracking-widest">{label}</span>
            </div>
            {/* Glow effect */}
            <div className={`absolute inset-0 rounded-full blur-xl opacity-20 pointer-events-none ${glowClass}`}></div>
        </div>
    );
};

const ActivityBars: React.FC<{ data: { s: number, e: number }[] }> = ({ data }) => {
        ...
        <div className="h-20 w-full px-2 py-2">
            <svg viewBox="0 0 120 80" className="w-full h-full">
                {recent.map((d, i) => {
                    ...
                    return (
                        <g key={i}>
                            <rect x={x} y={y} width="8" height={barH} rx="1" fill="rgba(51,65,85,0.3)" />
                            <rect x={x} y={80 - successH} width="8" height={successH} rx="1" fill="rgba(16,185,129,0.8)" />
                            <rect x={x} y={80 - successH - errorH} width="8" height={errorH} rx="1" fill="rgba(239,68,68,0.8)" />
                        </g>
                    )
                })}
            </svg>
...
const AreaSparkline: React.FC<{ data: number[], color: string, height?: number }> = ({ data, color, height = 40 }) => {
    ...
    return (
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible opacity-80" preserveAspectRatio="none">
            ...
            <path d={`M ${points} L 100,${height} L 0,${height} Z`} fill={`url(#grad-${color.replace('#', '')})`} stroke="none" />
            ...
            <circle cx="100" cy={height - ((data[data.length - 1] - min) / range) * height} r="2" fill={color} className="animate-pulse" />
        </svg>
    );
};
...
const ServiceCard: React.FC<{
    ...
    const statusColor = node.status === 'ONLINE' ? '#10b981' : node.status === 'DEGRADED' ? '#f59e0b' : '#ef4444';
    const statusText = node.status === 'ONLINE' ? 'text-emerald-500' : node.status === 'DEGRADED' ? 'text-amber-500' : 'text-red-500';
    const statusBg = node.status === 'ONLINE' ? 'bg-emerald-500/10 border-emerald-500/30' : node.status === 'DEGRADED' ? 'bg-amber-500/10 border-amber-500/30' : 'bg-red-500/10 border-red-500/30';
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 flex items-center justify-center w-full mb-2">
                {node.visualType === 'WAVE' && <div className="w-full h-16"><AreaSparkline data={metrics.lat} color={statusColor} /></div>}
                {node.visualType === 'GAUGE' && <RadialGauge value={metrics.load} label="COG LOAD" color={statusColor} />}
            ...
            <div className="flex justify-between items-end border-t border-slate-800 pt-2 mt-auto">
                ...
                <div className="text-right">
                    ...
                    <div className="text-xs font-mono font-bold text-slate-300">{currentTpt.toFixed(1)} /s</div>
                </div>
            </div>
...
const ShapeSelector: React.FC<{ current: CoreShape; onSelect: (s: CoreShape) => void }> = ({ current, onSelect }) => {
    ...
    return (
        <div className="grid grid-cols-4 gap-2">
            {shapes.map(s => (
                ...
                >
                    <div className={`w-3 h-3 rounded-sm ${current === s ? 'bg-cyan-400' : 'bg-gray-600'}`}></div>
                    <span className="text-[8px] uppercase font-bold tracking-wider">{s}</span>
                </button>
            ))}
        </div>
    );
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-800 text-slate-400 border border-slate-700">{node.type}</span>
                                {node.port && <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-blue-900/30 text-blue-400 border border-blue-500/30">PORT {node.port}</span>}
                            </div>
                        </div>
                    </div>
                    <button title="Close details" aria-label="Close details" onClick={onClose} className="p-2 hover:bg-slate-800 rounded-full transition-colors text-slate-500 hover:text-white">
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 rounded-xl bg-slate-900/30 border border-slate-800">
                            ...
                            <div className="text-[10px] text-slate-600 mt-2">Uptime: 99.98% • Last Restart: 4d ago</div>
                        </div>
                        <div className="p-4 rounded-xl bg-slate-900/30 border border-slate-800">
                    ...
                    <div className="space-y-4">
                        <div>
                            <div className="flex justify-between items-center mb-2">
                                <h3 className="text-xs font-bold text-slate-400">REAL-TIME LATENCY (ms)</h3>
                                <span className="text-xs font-mono text-emerald-500">{metrics.lat[metrics.lat.length - 1]?.toFixed(0)} ms</span>
                            </div>
                            <div className="h-32 bg-slate-900/50 rounded-lg border border-slate-800 p-2">
                                <AreaSparkline data={metrics.lat} color="#10b981" height={100} />
                            </div>
                        </div>
                    </div>

                    <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-800 font-mono text-[10px] text-slate-400">
                        <div className="flex gap-2 mb-1"><span className="text-slate-600">&gt;&gt;&gt;</span> Initializing handshake protocol... OK</div>
                        <div className="flex gap-2 mb-1"><span className="text-slate-600">&gt;&gt;&gt;</span> Authenticating security token... OK</div>
                        <div className="flex gap-2"><span className="text-slate-600">&gt;&gt;&gt;</span> Stream active. Listening on {node.port || 'IPC Channel'}...</div>
                    </div>
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <div className="text-sm font-bold text-slate-200">
                                        ...
                                        {mcpStatus?.pid ? <span className="text-slate-500 font-mono text-xs">  (pid {mcpStatus.pid})</span> : null}
                                    </div>
                                    {mcpStatus?.lastError ? <div className="text-[10px] text-red-400 font-mono mt-1">{mcpStatus.lastError}</div> : null}
                                </div>
...
export const SystemHub: React.FC<SystemHubProps> = ({ currentMode, setMode, coreConfig, setCoreConfig }) => {
    // Metric Simulation State
    const [metrics, setMetrics] = useState<Record<string, NodeMetrics>>({});
    const [selectedService, setSelectedService] = useState<ServiceNode | null>(null);
    const [mcpModules, setMcpModules] = useState<Record<string, McpModuleStatus> | null>(null);
    ...
    useEffect(() => {
        ...
        INVENTORY.forEach(node => {
            initialMetrics[node.id] = {
                lat: Array(20).fill(0).map(() => 20 + Math.random() * 30),
                tpt: Array(20).fill(0).map(() => Math.random() * 10),
                load: Math.random() * 100,
                activity: Array(20).fill(0).map(() => ({ s: Math.floor(Math.random() * 8), e: Math.floor(Math.random() * 2) }))
            };
        });
        ...
        const interval = setInterval(() => {
            setMetrics(prev => {
                const next = { ...prev };
                INVENTORY.forEach(node => {
                    const prevM = prev[node.id] || { lat: [], tpt: [], load: 0, activity: [] };

                    // Random Walk Logic
                    const lastLat = prevM.lat[prevM.lat.length - 1] || 30;
                    const newLat = Math.max(10, Math.min(200, lastLat + (Math.random() - 0.5) * 20));

                    const lastTpt = prevM.tpt[prevM.tpt.length - 1] || 5;
                    const newTpt = Math.max(0, Math.min(50, lastTpt + (Math.random() - 0.5) * 5));

                    const newLoad = Math.max(0, Math.min(100, prevM.load + (Math.random() - 0.5) * 10));
                    const newActivity = { s: Math.floor(Math.random() * 8), e: Math.random() > 0.8 ? 1 : 0 };
    ...
    const selectedMcpName = selectedService?.type === 'MCP' ? MCP_ID_TO_NAME[selectedService.id] : null;
    const selectedMcpStatus = selectedMcpName ? (mcpModules?.[selectedMcpName] || null) : null;
    ...
    const [activeSubTab, setActiveSubTab] = useState<'OVERVIEW' | 'NETWORKING' | 'DEPLOYMENT'>('OVERVIEW');
        ...
        <div className="h-full overflow-y-auto p-4 md:p-8 space-y-8 bg-black/40 custom-scrollbar animate-[fade-in_0.4s_ease-out] pb-32">
            ...
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 px-2">
                <div>
                    <h2 className="text-3xl font-bold text-white tracking-widest uppercase flex items-center gap-4">
                        ...
                        Agent Lee OS <span className="text-blue-500/50 text-base font-light">CORE v4.0</span>
                    </h2>
                    <p className="text-gray-400 text-sm mt-2 font-mono tracking-tight uppercase">Sovereign Orchestration Layer • Hardened Kernel • 884-AX</p>
                </div>
            ...
            {activeSubTab === 'OVERVIEW' && (
                <div className="space-y-10">
                    ...
                    <HUDFrame title="Vortex Core Matrix" icon={<Grid size={14} className="text-cyan-400" />}>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-2">
                            <div className="space-y-8">
                                <ShapeSelector
                                    current={coreConfig.shape}
                                    onSelect={(s) => setCoreConfig(prev => ({ ...prev, shape: s, autoMorphEnabled: false }))}
                                />
                                ...
                                <div className="flex items-center justify-between p-5 bg-gray-900/40 rounded-xl border border-gray-800 hover:border-blue-500/30 transition-all">
                                    <div className="flex items-center gap-4">
                                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors ${coreConfig.autoMorphEnabled ? 'bg-green-500 text-black shadow-[0_0_15px_rgba(34,197,94,0.4)]' : 'bg-gray-800 text-gray-500'}`}>
                                            <RotateCw size={22} className={coreConfig.autoMorphEnabled ? 'animate-spin' : ''} />
                                        </div>
                                        <div>
                                            <span className="text-sm font-bold text-gray-200 block">Auto-Sequence</span>
                                            <span className="text-xs text-gray-500 block mt-0.5">Autonomous shape shifting</span>
                                        </div>
                                    </div>
                                    <button
                                        onClick={() => setCoreConfig(prev => ({ ...prev, autoMorphEnabled: !prev.autoMorphEnabled }))}
                                        className={`w-14 h-7 rounded-full relative transition-all duration-300 ${coreConfig.autoMorphEnabled ? 'bg-green-600' : 'bg-gray-700'}`}
                                        aria-label="Toggle auto sequence"
                                    >
                                        <div className={`absolute top-1 w-5 h-5 rounded-full bg-white shadow-sm transition-all duration-300 ${coreConfig.autoMorphEnabled ? 'left-8' : 'left-1'}`}></div>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6 pt-2">
                                <CoreControl
                                    ...
                                    value={coreConfig.morphSpeed} min={1.0} max={10.0} step={0.5}
                                    onChange={(v) => setCoreConfig(prev => ({ ...prev, morphSpeed: v }))}
                                />
                                <CoreControl
                                    label="Particle Density"
                                    icon={<Grid size={14} className="text-purple-400" />}
                                    value={coreConfig.density} min={1000} max={20000} step={500}
                                    onChange={(v) => setCoreConfig(prev => ({ ...prev, density: v }))}
                                />
                                <CoreControl
                                    label="Luminosity"
                                    icon={<Sun size={14} className="text-yellow-400" />}
                                    value={coreConfig.brightness} min={0.2} max={2.0} step={0.1}
                                    onChange={(v) => setCoreConfig(prev => ({ ...prev, brightness: v }))}
                                />
                            </div>
                        </div>
                    </HUDFrame>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {inventoryWithLiveStatus.map(node => (
                                <ServiceCard
                                    key={node.id}
                                    node={node}
                                    metrics={metrics[node.id] || { lat: [], tpt: [], load: 0, activity: [] }}
            ...
            {activeSubTab === 'NETWORKING' && (
                <div className="space-y-8 max-w-5xl mx-auto">
                    <HUDFrame title="REMOTE ACCESS & TUNNELS" icon={<Radio size={16} className="text-orange-500" />}>
                        <div className="bg-black/30 rounded-2xl overflow-hidden border border-gray-800 shadow-2xl">
                            <TunnelPanel />
                        </div>
                    </HUDFrame>

                    <HUDFrame title="NETWORK DIAGNOSTICS" icon={<Network size={16} className="text-blue-500" />}>
                        <div className="bg-gray-900/40 p-8 rounded-2xl border border-gray-800 text-gray-400 text-sm font-mono leading-relaxed space-y-4">
                            <div className="flex items-center gap-4">
                                ...
                                <span className="text-gray-500">127.0.0.1:8001 (LISTENING)</span>
                            </div>
                            <div className="flex items-center gap-4">
                                <span className="w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse"></span>
                                <span className="text-white font-bold w-32">Neural Router:</span>
                                <span className="text-gray-500">127.0.0.1:8004 (ACTIVE)</span>
                            </div>
                            <div className="flex items-center gap-4">
                                <span className="w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse"></span>
                                <span className="text-white font-bold w-32">Desktop Agent:</span>
                                <span className="text-gray-500">127.0.0.1:8005 (READY)</span>
                            </div>
                            <div className="mt-8 pt-8 border-t border-gray-800/50">
                                <div className="flex items-center justify-between">
                                    <span className="text-xs text-gray-600 uppercase tracking-widest font-bold">Encrypted via RSA-4096 / SHA-256</span>
                                    <span className="text-emerald-500 text-xs font-mono uppercase">Secure Link Active</span>
                                </div>
                            </div>
                        </div>
                    </HUDFrame>
            ...
            {selectedService && (
                <ServiceDetailModal
                    ...
                    metrics={metrics[selectedService.id] || { lat: [], tpt: [], load: 0, activity: [] }}
                    onClose={() => {
