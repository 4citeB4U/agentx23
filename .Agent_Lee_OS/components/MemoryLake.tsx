...
const DRIVE_COLORS: Record<DriveId, string> = {
    "C": "#60a5fa", "LEE": "#ffffff", "N": "#d8b4fe", "A": "#f472b6", "R": "#fb923c",
...
class NeuralLink {
    ...
    private handshake = (import.meta as any).env?.VITE_NEURAL_HANDSHAKE as string | undefined;
    ...
    private async initializeCore() {
        ...
        if (!meta) {
            console.log("Agent Lee: Initializing Memory Lake (Explorer Mirror Mode)...");
    ...
    private async getSlotRoots(driveId: DriveId): Promise<string[]> {
        ...
        const r = await this.fsFetch(`/api/fs/list?root=${encodeURIComponent(driveId)}&path=`);
        if (!r.ok) throw new Error(`FS_LIST_HTTP_${r.status}`);
        const data = await r.json();
        const dirs = (Array.isArray(data?.entries) ? data.entries : [])
            .filter((e: any) => e?.type === 'directory' && typeof e?.name === 'string')
    ...
    private inferCategory(name: string): FileCategory {
        ...
        if (/(\.ts|\.tsx|\.js|\.jsx|\.json|\.py|\.cs|\.java|\.md|\.yml|\.yaml|\.toml|\.ini)$/i.test(lower)) return 'code';
        if (/(\.png|\.jpg|\.jpeg|\.gif|\.webp|\.svg|\.mp3|\.wav|\.mp4|\.webm|\.mov)$/i.test(lower)) return 'media';
    ...
    async getFiles(driveId: DriveId, slotId: number): Promise<NeuralFile[]> {
        ...
        const r = await this.fsFetch(`/api/fs/list?root=${encodeURIComponent(driveId)}&path=${encodeURIComponent(bucket)}`);
        if (!r.ok) return [];
        const data = await r.json();
    ...
    async deleteFile(id: string) {
        ...
        const rel = parts.slice(1).join(':');
        if (!driveId || !rel) return;
        const r = await this.fsFetch(`/api/fs/delete?root=${encodeURIComponent(driveId)}&path=${encodeURIComponent(rel)}`, { method: 'DELETE' });
    ...
    return (
        <div ref={setRef(`file_${file.id}`)} onClick={handleClick} draggable={!isEditing}
            ...
            <div className="flex justify-between items-start">
                <div className={`w-2 h-2 rounded-full ${isOffloaded ? 'bg-slate-600 animate-pulse' : ''} ${isCorrupt ? 'bg-red-500 animate-ping' : ''} ${(!isOffloaded && !isCorrupt) ? DRIVE_BG_CLASS[activeDriveId] : ''}`} />
                ...
                {(!isOffloaded && !isCorrupt && !selectionMode && fileRelations[file.id]) && (
                    <div className="flex gap-1.5 bg-black/50 rounded-full px-1.5 py-0.5 border border-white/10">
                        {fileRelations[file.id].map(r => <div key={r} className={`w-2 h-2 rounded-full shadow-[0_0_5px_currentColor] animate-pulse ${DRIVE_BG_CLASS[r]} ${DRIVE_TEXT_CLASS[r]}`} />)}
                    </div>
                )}
            </div>
            ...
            <div className="mt-2 z-10 relative">
                {isEditing ? (
                    <input ref={inputRef} value={editName} onChange={e => setEditName(e.target.value)} onBlur={handleRenameSubmit} onKeyDown={handleKeyDown} onClick={e => e.stopPropagation()} title="Rename file" aria-label="Rename file" placeholder="Rename file" className="w-full bg-black/50 border border-emerald-500/50 text-xs font-bold text-white px-1 rounded outline-none font-mono" />
                ) : (
...
const MediaPreview: React.FC<{ file: NeuralFile }> = ({ file }) => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            if (file.content instanceof Blob) blob = file.content;
                ...
                try {
                    const r = await fetch(`/api/fs/read?root=${encodeURIComponent(file.driveId)}&path=${encodeURIComponent(file.externalRef.path)}&ts=${Date.now()}`,
                        {
                            headers: {
    ...
    }, [file.id, file.status, file.externalRef?.archiveId, file.externalRef?.path, file.driveId]);
...
const MemoryLakeCore = forwardRef<AgentLeeRef, { onExit?: () => void }>(({ onExit }, ref) => {
    ...
    const handleExport = async (scope: 'file' | 'slot' | 'drive' | 'system' | 'selected', targetFile?: NeuralFile) => {
        ...
        for (const f of targets) {
            ...
            if (f.externalRef?.type === 'handle') {
                try {
                    const r = await fetch(`/api/fs/read?root=${encodeURIComponent(f.driveId)}&path=${encodeURIComponent(f.externalRef.path)}&ts=${Date.now()}`,
                        {
                            headers: {
                                ...(handshake ? { 'x-neural-handshake': handshake } : {}),
                                'ngrok-skip-browser-warning': '1'
                    ...
                    if (r.ok) {
                        ...
                        zip.file(`${entryName}.error.txt`, `FAILED_TO_READ_REMOTE_FILE: HTTP ${r.status}`);
                    }
                } catch (e) {
                    zip.file(`${entryName}.error.txt`, `FAILED_TO_READ_REMOTE_FILE: ${String(e).slice(0, 200)}`);
                }
            }
        }
            ...
            <div className="w-full max-w-lg bg-slate-950 border border-slate-800 rounded-xl shadow-2xl overflow-hidden animate-[fade-in_0.2s_ease-out] z-[60]">
                <div className="p-6 border-b border-slate-900 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        <div className={`w-3 h-3 rounded-full ${DRIVE_BG_CLASS[activeDriveId]}`} />
                ...
                <div className="p-8 flex flex-col items-center justify-center min-h-[150px] bg-slate-900/30">
                    ...
                    <div className="grid grid-cols-2 gap-3 w-full max-w-xs">
                        ...
                        {activeFile.externalRef?.type !== 'handle' ? (
                            ...
                            <div className="py-3 border border-slate-800 text-slate-600 text-xs font-bold rounded flex items-center justify-center">REMOTE FILE</div>
                        )}
                    </div>
                </div>
    ...
    return (
        <div ref={containerRef} className="relative w-full h-[100dvh] bg-black overflow-hidden flex flex-col text-slate-200 select-none font-sans" onDragOver={(e) => e.preventDefault()} onDrop={handleSlotDrop}>
            <label id="memorylake-upload-files-label" htmlFor="memorylake-upload-files" className="sr-only">Upload files</label>
            <input id="memorylake-upload-files" name="memorylake-upload-files" type="file" ref={fileInputRef} multiple className="hidden" onChange={handleUpload} title="Upload files" aria-label="Upload files" aria-labelledby="memorylake-upload-files-label" />
            <label id="memorylake-upload-folder-label" htmlFor="memorylake-upload-folder" className="sr-only">Upload folder</label>
            <input
            ...
            <div className="z-30 min-h-16 py-2 flex flex-col md:flex-row items-center justify-between px-2 md:px-6 bg-gradient-to-b from-black via-black/95 to-transparent gap-2 md:gap-0">
                <div className="w-full md:w-auto flex justify-between items-center">
                    <div className="flex flex-col"><h1 className="text-xs md:text-sm font-black tracking-[0.3em] uppercase text-slate-400">Agent Lee</h1><div className="text-[9px] text-emerald-500 font-mono">NEURAL MEMORY V8.4 (MONOLITH)</div></div>
                    {/* New Back Button */}
                    {onExit && (
                        <button onClick={onExit} className="ml-4 px-3 py-1 border border-slate-700 bg-slate-900/50 text-slate-400 text-[10px] font-bold rounded hover:bg-slate-800 hover:text-white transition-colors flex items-center gap-2">
                            <span>←</span> BACK
                        </button>
                    )}
                    <div className="flex gap-2 md:hidden"><button onClick={() => handleExport('drive')} className="px-2 py-1 rounded border border-slate-700 bg-slate-900/50 text-[9px] hover:bg-slate-800 text-slate-400">DRIVE</button></div>
                </div>
                    ...
                    {activeSlotId ? (
                        <>
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-white/5 pb-4 mb-4 gap-4">
                                <div><h2 className={`text-2xl md:text-3xl font-black tracking-tighter ${DRIVE_TEXT_CLASS[activeDriveId]}`}>SECTOR 0{activeSlotId}</h2><p className="text-[10px] md:text-xs text-slate-500 font-mono mt-1">{files.length} NODES • {files.filter(f => f.status === 'offloaded').length} EXTERNAL • {files.filter(f => f.status === 'corrupt').length} CORRUPT</p></div>
...
const MemoryLake: React.FC = () => {
    ...
    useEffect(() => {
        if (isLakeOpen && agentRef.current) {
            (window as any).agentLee = agentRef.current;
            console.log("Agent Lee Monolith Online");
        }
    }, [isLakeOpen]);

    if (!isLakeOpen) {
        return (
            <div className="w-full h-[100dvh] bg-black flex flex-col items-center justify-center text-slate-200 overflow-hidden relative selection:bg-cyan-500/30">
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-slate-900/20 via-black to-black pointer-events-none" />

                <div className="absolute w-[600px] h-[600px] border border-slate-900/50 rounded-full animate-[spin_20s_linear_infinite]" />
                <div className="absolute w-[400px] h-[400px] border border-slate-800/50 rounded-full animate-[spin_15s_linear_infinite_reverse]" />

                <div className="z-10 flex flex-col items-center">
                    <h1 className="text-5xl md:text-7xl font-black tracking-[0.2em] mb-2 text-transparent bg-clip-text bg-gradient-to-b from-white to-slate-600 uppercase">Agent Lee</h1>
                    <div className="text-xs font-mono text-cyan-500 tracking-[0.5em] mb-12 opacity-70">NEURAL INTERFACE V8.5</div>

                    <button
                        onClick={() => setIsLakeOpen(true)}
                        className="group relative px-12 py-5 bg-black border border-slate-800 text-slate-300 font-mono tracking-widest text-sm hover:text-white transition-all duration-300 overflow-hidden"
                    >
                        <span className="absolute inset-0 w-full h-full bg-cyan-900/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out"></span>
    ...
    return (
        <div className="antialiased text-slate-100 bg-black min-h-screen">
            <MemoryLakeCore ref={agentRef} onExit={() => setIsLakeOpen(false)} />
        </div>
    );
};
