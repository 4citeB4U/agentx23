import { AlertTriangle, Brain, Cpu, HardDrive, Shield, TrendingUp, Zap } from 'lucide-react';
...
export const SystemTelemetry: React.FC = () => {
    ...
    const [tab, setTab] = useState<'ports' | 'adapters' | 'missions' | 'drift'>('ports');
    ...
    const adapterEntries = brain ? (Object.entries(brain.adapter_performance || {}) as [string, AdapterStat][]) : [];
    const bestAdapter = adapterEntries.sort(([, a], [, b]) => b.avg_reward - a.avg_reward)[0];
    ...
    return (
        <div className="bg-black/80 backdrop-blur-xl border border-studio-border/30 rounded-2xl p-4 w-80 shadow-2xl space-y-3">
            ...
            <div className="flex items-center justify-between border-b border-studio-border/20 pb-2">
                ...
                <div className={`px-2 py-0.5 rounded-full text-[8px] font-bold uppercase ${data.overallStatus === 'nominal'
            ...
            {brain && (
                <div className="grid grid-cols-3 gap-1.5">
                    <div className="bg-white/5 rounded-lg p-1.5 text-center">
                        <div className="text-[16px] font-bold text-cyan-400">{brain.episodes_today}</div>
                        <div className="text-[8px] text-studio-secondary/60 font-mono uppercase">Today</div>
                    </div>
                    <div className="bg-white/5 rounded-lg p-1.5 text-center">
                        <div className="text-[16px] font-bold text-purple-400">{brain.episodes_total}</div>
                        <div className="text-[8px] text-studio-secondary/60 font-mono uppercase">Total</div>
                    </div>
                    <div className="bg-white/5 rounded-lg p-1.5 text-center">
                        <div className="text-[16px] font-bold text-emerald-400">{brain.synthetic_generated}</div>
                        <div className="text-[8px] text-studio-secondary/60 font-mono uppercase">Synthetic</div>
                    </div>
                </div>
            )}
            ...
            <div className="flex gap-1 bg-white/5 rounded-lg p-0.5">
                {(['ports', 'adapters', 'missions', 'drift'] as const).map(t => (
                    <button
                        key={t}
                        onClick={() => setTab(t)}
                        className={`flex-1 text-[8px] font-mono uppercase py-1 rounded transition-all ${tab === t
                            ? 'bg-cyan-400/20 text-cyan-400'
            ...
            {tab === 'ports' && (
                <div className="space-y-1.5">
                    <div className="flex items-center gap-2 text-[9px] text-studio-secondary/60 uppercase font-mono mb-1">
                        <HardDrive size={10} /><span>Port_Enforcement</span>
                    </div>
                    {data.services.map(s => (
                        <div key={s.port} className="flex items-center justify-between text-[10px] bg-white/5 p-1.5 rounded-lg">
                            <span className="text-studio-secondary">{s.name} ({s.port})</span>
                            <span className={s.status === 'online' ? 'text-cyan-400' : 'text-red-500'}>
                    ...
                    <div className="flex items-center justify-between text-[10px] bg-white/5 p-1.5 rounded-lg pt-2 mt-1 border-t border-studio-border/10">
                        ...
                        <span className={`font-mono uppercase ${brain?.voice_state === 'PRIMARY' ? 'text-cyan-400' : 'text-amber-400'}`}>
                            {brain?.voice_state || '—'}
                        </span>
                    </div>
            ...
            {tab === 'adapters' && (
                <div className="space-y-1.5">
                    <div className="flex items-center gap-2 text-[9px] text-studio-secondary/60 uppercase font-mono mb-1">
                        <Cpu size={10} /><span>Adapter_Performance</span>
                    </div>
                    ...
                    {adapterEntries.map(([adapter, stat]) => (
                        <div key={adapter} className="bg-white/5 rounded-lg p-2 space-y-1">
                            <div className="flex items-center justify-between">
                                <span className={`text-[10px] font-mono font-bold ${ADAPTER_COLOR[adapter] || 'text-white'}`}>
                                    {ADAPTER_SHORT[adapter] || adapter}
                                </span>
                                <span className="text-[9px] text-studio-secondary/40">{stat.episodes} eps</span>
                            </div>
                            <div className="grid grid-cols-2 gap-1">
                                <div className="flex justify-between text-[9px]">
                                    <span className="text-studio-secondary/50">Reward</span>
                                    <span className={rewardColor(stat.avg_reward)}>{stat.avg_reward.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between text-[9px]">
                                    <span className="text-studio-secondary/50">Success</span>
                                    <span className="text-emerald-400">{(stat.success_rate * 100).toFixed(0)}%</span>
                                </div>
                                <div className="flex justify-between text-[9px]">
                                    <span className="text-studio-secondary/50">Depth</span>
                                    <span className="text-purple-400">{stat.avg_planning_depth.toFixed(1)}</span>
                                </div>
                                <div className="flex justify-between text-[9px]">
                                    <span className="text-studio-secondary/50 flex items-center gap-0.5">
                                        <AlertTriangle size={7} />Halluc
                                    </span>
                                    <span className={stat.total_hallucinations > 0 ? 'text-red-400' : 'text-emerald-400'}>
                    ...
                    {bestAdapter && (
                        <div className="flex items-center gap-1.5 pt-1 border-t border-studio-border/10">
                            ...
                            <span className={`text-[9px] font-mono ${ADAPTER_COLOR[bestAdapter[0]] || 'text-white'}`}>
                                {ADAPTER_SHORT[bestAdapter[0]]} ({bestAdapter[1].avg_reward.toFixed(2)} R)
                            </span>
                        </div>
                    )}
                    {brain && (
            ...
            {tab === 'missions' && (
                <div className="space-y-1.5">
                    <div className="flex items-center gap-2 text-[9px] text-studio-secondary/60 uppercase font-mono mb-1">
                        <TrendingUp size={10} /><span>Last_10_Missions</span>
                    </div>
                    ...
                    {(brain?.last_10_missions || []).map((m, i) => (
                        <div key={i} className="flex items-center justify-between text-[9px] bg-white/5 p-1.5 rounded-lg">
                            <div className="flex flex-col gap-0.5">
                                <span className={`font-mono uppercase ${ADAPTER_COLOR[m.adapter] || 'text-white'}`}>
                                    {ADAPTER_SHORT[m.adapter] || (m.adapter || '?').slice(0, 8)}
                                </span>
                                <span className="text-studio-secondary/40 text-[8px]">{m.domain}</span>
                            </div>
                            <div className="flex flex-col items-end gap-0.5">
                                <span className={rewardColor(m.reward || 0)}>
                                    R={((m.reward || 0)).toFixed(2)}
                                </span>
                                <span className={`text-[8px] uppercase ${m.outcome === 'ok' ? 'text-emerald-400/70'
                                    : m.outcome === 'fallback' ? 'text-amber-400/70'
            ...
            {tab === 'drift' && (
                <div className="space-y-2">
                    <div className="flex items-center gap-2 text-[9px] text-studio-secondary/60 uppercase font-mono mb-1">
                        <TrendingUp size={10} /><span>Learning_Drift · 7d</span>
                    </div>
                    {(!drift?.trend?.length) && (
                        <div className="text-[10px] text-studio-secondary/40 text-center py-3 font-mono">
                    ...
                    {(drift?.trend || []).map((d) => {
                        ...
                        return (
                            <div key={d.day} className="bg-white/5 rounded-lg p-2 space-y-1">
                                <div className="flex justify-between text-[9px]">
                                    <span className="text-studio-secondary/60 font-mono">{d.day.slice(5)}</span>
                                    <span className="text-studio-secondary/40">{d.total_episodes} eps</span>
                                </div>
                                {/* Reward bar */}
                                <div className="flex items-center gap-1.5">
                                    <span className="text-[8px] text-studio-secondary/40 w-10">Reward</span>
                                    <progress
                                        className={`flex-1 telemetry-progress ${d.avg_reward >= 0.8 ? 'bar-emerald'
                                            : d.avg_reward >= 0.5 ? 'bar-amber'
                                                : 'bar-red'
                                            }`}
                                        value={barW}
                                        max={100}
                                    />
                                    <span className={`text-[8px] font-mono ${rewardColor(d.avg_reward)}`}>
                                ...
                                <div className="grid grid-cols-3 gap-1">
                                    <div className="flex flex-col items-center">
                                        <span className={`text-[10px] font-mono ${d.avg_halluc > 1 ? 'text-red-400'
                                            : d.avg_halluc > 0 ? 'text-amber-400'
                                                : 'text-emerald-400'
                                            }`}>{d.avg_halluc.toFixed(1)}</span>
                                        <span className="text-[7px] text-studio-secondary/30">Halluc</span>
                                    </div>
                                    <div className="flex flex-col items-center">
                                        <span className="text-[10px] font-mono text-purple-400">{d.avg_depth.toFixed(1)}</span>
                                        <span className="text-[7px] text-studio-secondary/30">Depth</span>
                                    </div>
                                    <div className="flex flex-col items-center">
                                        <span className="text-[10px] font-mono text-cyan-400">{(d.synthetic_ratio * 100).toFixed(0)}%</span>
                                        <span className="text-[7px] text-studio-secondary/30">Synth</span>
                                    </div>
                                </div>
                            </div>
                        );
                    ...
                    {/* Adapter dominance */}
                    {drift?.adapter_dominance && Object.keys(drift.adapter_dominance).length > 0 && (
                        <div className="pt-1 border-t border-studio-border/10 space-y-1">
                            <div className="text-[8px] text-studio-secondary/40 font-mono uppercase">Adapter Dominance</div>
                            {(Object.entries(drift.adapter_dominance) as [string, number][]).map(([adapter, share]) => (
                                <div key={adapter} className="flex items-center gap-1.5">
                                    <span className={`text-[8px] font-mono w-14 ${ADAPTER_COLOR[adapter] || 'text-white'}`}>
                                        {ADAPTER_SHORT[adapter] || adapter.slice(0, 8)}
                                    </span>
                                    <progress
                                        className={`flex-1 telemetry-progress ${adapter.includes('general') ? 'bar-cyan'
                                            : adapter.includes('code') ? 'bar-emerald'
                                                : adapter.includes('ui') ? 'bar-purple'
                                                    : adapter.includes('cdl') ? 'bar-amber'
                                                        : 'bar-white'
                                            }`}
                                        value={Math.round(share * 100)}
                                        max={100}
                                    />
                                    <span className="text-[8px] text-studio-secondary/50">{(share * 100).toFixed(0)}%</span>
                                </div>
                            ))}
                        </div>
                    )}
            ...
            <div className="pt-1 flex justify-between items-center border-t border-studio-border/10">
                <span className="text-[8px] text-studio-secondary/30 font-mono">v3 · LOOP_ACTIVE</span>
