...
export const PhoneView: React.FC = () => {
    const [status, setStatus] = useState<PhoneStatus>({ bridgeOnline: false, deviceConnected: false });
    ...
    const fetchStatus = async () => {
        try {
            const handshake = import.meta.env.VITE_NEURAL_HANDSHAKE || '';
            ...
            if (r.ok) {
                ...
                setStatus({ bridgeOnline: false, deviceConnected: false, error: `HTTP ${r.status}` });
            }
        } catch {
            setStatus({ bridgeOnline: false, deviceConnected: false, error: 'Bridge offline' });
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="flex flex-col h-full w-full bg-black/30 rounded-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-white/10 bg-white/[0.03]">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] font-bold tracking-widest uppercase text-cyan-400">Phone Mirror</span>
                </div>
                <div className="flex items-center gap-2">
                    {/* Bridge status */}
                    <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-mono ${status.bridgeOnline ? 'border-green-500/40 text-green-400 bg-green-500/10' : 'border-red-500/40 text-red-400 bg-red-500/10'}`}>
                        {status.bridgeOnline ? <Wifi size={10} /> : <WifiOff size={10} />}
                        {status.bridgeOnline ? 'bridge online' : 'bridge offline'}
                    </div>
                    {/* Device status */}
                    {status.bridgeOnline && (
                        <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-mono ${status.deviceConnected ? 'border-cyan-500/40 text-cyan-400 bg-cyan-500/10' : 'border-gray-500/40 text-gray-500'}`}>
                            <Usb size={10} />
                            {status.deviceConnected ? (status.deviceName || 'device connected') : 'no device'}
                        </div>
                    )}
                </div>
            </div>
                        ...
                        <div className="max-w-md w-full space-y-6">
                            <div className="text-center">
                                ...
                                <p className="text-gray-400 text-sm leading-relaxed">
                                    The <span className="text-cyan-400 font-mono">AgentLee-PhoneBridge</span> PM2 service is offline.
                                </p>
                            </div>
                            <div className="bg-black/40 border border-white/10 rounded-xl p-5 space-y-3">
                                <p className="text-[10px] font-bold tracking-widest uppercase text-gray-500">Setup — run once</p>
...
adb tcpip 5555`}</pre>
                        ...
                        <div className="max-w-sm w-full space-y-4">
                            <div className="text-center">
                                ...
                                <p className="text-gray-400 text-sm">Enter your phone's Wi-Fi IP to connect via ADB</p>
                            </div>
                            ...
                            {connectError && (
                                <p className="text-red-400 text-xs font-mono text-center">{connectError}</p>
                            )}
                        </div>
                    )}
                </div>
