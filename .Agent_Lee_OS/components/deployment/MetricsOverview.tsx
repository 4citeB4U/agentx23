...
import { Activity, AlertCircle, CheckCircle, Server, Users, Zap } from 'lucide-react';
...
export const MetricsOverview: React.FC<Props> = ({ apps }) => {
  ...
  const errors   = apps.filter((a) => a.status === 'error' || a.status === 'degraded').length;
  const stopped  = apps.filter((a) => a.status === 'stopped').length;
  const totalRPS = apps.reduce((s, a) => s + a.metrics.rps, 0);
  const avgCPU   = (apps.reduce((s, a) => s + a.metrics.cpu, 0) / apps.length).toFixed(0);
  ...
  const tiles: Tile[] = [
    ...
    },
    {
      ...
      glow: Number(avgCPU) > 80 ? 'rgba(239,68,68,0.15)' : 'rgba(245,158,11,0.15)',
    },
    {
      icon: <Users size={14} />,
      label: 'Live Users',
  ...
  return (
    <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-4">
      {tiles.map((t) => (
        <div
          key={t.label}
          className="flex flex-col gap-1 rounded-xl border border-white/8 p-3"
          style={{ background: t.glow }}
        >
          <div className={`flex items-center gap-1.5 ${t.color}`}>
            {t.icon}
            <span className="text-[9px] font-mono tracking-widest text-white/40 uppercase">{t.label}</span>
          </div>
          <span className={`text-lg font-bold font-mono ${t.color}`}>{t.value}</span>
        </div>
      ))}
    </div>
  );
