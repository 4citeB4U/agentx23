...
## Keeps desktop_agent.py alive; checks port 6005 every 5 s and restarts if dead.
param(
...
function IsAgentAlive {
    $listening = netstat -ano 2>$null | Select-String ":6005 .*LISTEN"
...
while ($true) {
    if (-not (IsAgentAlive)) {
        ...
        Write-Host "[$ts][watchdog] Port 6005 dead – restarting agent..."
        Add-Content "$WorkDir\$LogOut" "[$ts][watchdog] Restarting desktop agent"
        # Kill any stale process first
        netstat -ano 2>$null | Select-String ":6005 " | ForEach-Object {
            $pid = ($_ -split '\s+')[-1]
