// Puppeteer CommonJS smoke test for environments with package.json type: module
// Usage: npm i puppeteer && node scripts/puppeteer-check-ui.cjs
...
(async () => {
  ...
  const browser = await puppeteer.launch({
    headless: true,
    args: ["--no-sandbox", "--explicitly-allowed-ports=6000"],
  });
  ...
  try {
    ...
    const bottomNavExists =
      (await page.$('.bottom-nav, [data-test="bottom-nav"], #bottom-nav')) !==
      null;
    const micButtonExists =
      (await page.$(
        '.mic-button, [data-test="mic"], button[aria-label*="mic"]',
      )) !== null;
    ...
    // Save console logs
    fs.writeFileSync(
      path.resolve("scripts", "puppeteer-console.json"),
