...
const BASE_URL = process.env.AGENT_LEE_URL || 'http://localhost:8001';
...
async function waitForBodyText(substring, timeout = HYDRATE_TIMEOUT) {
  ...
  const final = await page.evaluate(() => document.body.innerText).catch(() => '');
  throw new Error(`"${substring}" not found within ${timeout}ms. Body snippet: "${final.slice(0, 200)}"`);
}
...
async function run() {
  ...
  page.on('response', r => {
    if (r.status() >= 500) networkFailures.push(`${r.status()} ${r.url().replace(BASE_URL, '')}`);
  });
  page.on('pageerror', e => console.log(`  [js-error] ${e.message.slice(0, 120)}`));

  // ── 1. Page loads and React hydrates ─────────────────────────────────────
  await test('Page loads (200 HTML)', async () => {
    const res = await page.goto(BASE_URL, { waitUntil: 'domcontentloaded', timeout: NAV_TIMEOUT });
  ...
  await test('React mounts (#root has content)', async () => {
    ...
    if (networkFailures.length > 0) {
      throw new Error(`Asset 500 errors: ${networkFailures.slice(0,3).join(', ')} — possibly backend still starting`);
    }
    throw new Error('#root is empty after 12s — JS bundle failed to load or execute');
  });

  if (networkFailures.length > 0) {
    console.log(`  [warn] ${networkFailures.length} asset errors (likely transient on startup):`);
  ...
  await test('VoxelCore renders (canvas or headless fallback)', async () => {
    ...
    const result = await page.evaluate(() => {
      ...
      const fallback = Array.from(document.querySelectorAll('div')).find(d => d.textContent.includes('VOXEL CORE'));
      if (fallback) return 'fallback';
      return null;
    });
    assert(result !== null, 'Neither <canvas> WebGL nor headless fallback div found — VoxelCore not mounting');
  });
  ...
  await test('Command input is present', async () => {
    ...
    assert(input !== null, 'No input/textarea with placeholder found on COMMS tab');
  });
  ...
  await test('Agent response appears (no raw error text)', async () => {
    ...
    while (Date.now() < deadline) {
      const txt = await page.evaluate(() => document.body.innerText).catch(() => '');
      const hasReply = txt.includes('Yo') || txt.includes('real talk') || txt.includes('neural bridge') ||
                       txt.includes('COMMS') || txt.includes('Agent Lee') || txt.includes('fam');
      const hasCrash = txt.includes('Unexpected token') || txt.includes('SyntaxError');
  ...
  await test('Mic button exists (SVG mic icon in CommandInput)', async () => {
    ...
    assert(micFound, 'No icon-only button with SVG found - check CommandInput onMicToggle wiring');
  });
  ...
  await test('Can navigate back to Home/COMMS (aria-label="Home")', async () => {
    ...
    const input = await page.$('input[placeholder], textarea[placeholder]');
    assert(input !== null, 'Command input not visible after returning to Home (COMMS)');
  });
