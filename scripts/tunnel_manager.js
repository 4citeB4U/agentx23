...
const CLOUDFLARED = process.env.CLOUDFLARED_PATH || 'C:\\Tools\\cloudflared.exe';
const LOCAL_PORT = Number(process.env.TARGET_PORT || process.env.PORT || 8000);
...
function startTunnel() {
    ...
    tunnelProc = spawn(CLOUDFLARED, ['tunnel', '--url', `http://localhost:${LOCAL_PORT}`], {
    ...
    const handleLine = (line) => {
        ...
        const urlMatch = line.match(/https:\/\/[a-z0-9-]+\.trycloudflare\.com/i)
            || line.match(/https:\/\/[a-z0-9.-]+\.(cfargotunnel\.com|trycloudflare\.com|cloudflare\.com)[^\s"]*/i);
        if (urlMatch) {
    ...
    tunnelProc.on('exit', (code, signal) => {
        log(`cloudflared exited (code=${code}, signal=${signal}). Restarting in 5s...`);
        publicUrl = null;
...
const server = http.createServer((req, res) => {
    ...
    if (req.url === '/status') {
        res.writeHead(200);
        res.end(JSON.stringify({ active: !!publicUrl, url: publicUrl, port: LOCAL_PORT }));
        return;
    }
