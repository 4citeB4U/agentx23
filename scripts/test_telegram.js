 ...
 *   Backend running on 8001 with valid handshake
 */
...
async function getLatestUpdates(limit = 10) {
  const r = await fetch(`${TG_API}/getUpdates?limit=${limit}&allowed_updates=["message"]`);
...
async function run() {
  ...
  await test('POST /api/chat sends without error', async () => {
    const r = await fetch(`${BASE}/api/chat`, {
      ...
      body: JSON.stringify({ text: `Yo this is a test ping ${TAG}`, id: TAG, source: 'web' })
    });
  ...
  while (Date.now() < deadline) {
    ...
    for (const u of fresh) {
      ...
      if (text.includes(TAG) && !userMsgFound && (text.includes('User:') || text.includes('Voice:') || text.includes(TAG))) {
        userMsgFound = true;
  ...
  await test('User message appeared on Telegram', () => {
    assert(userMsgFound, `No Telegram message containing "${TAG}" found within 30s`);
  });

  if (userMsgFound && userMsgTime) {
    const relayDelay = userMsgTime - sendTime;
    console.log(`  ⏱  User message relay delay: ${relayDelay}ms (target < 2000ms)`);
    await test('User message arrived within 2 seconds of send', () => {
      assert(relayDelay < 2000, `Relay took ${relayDelay}ms — expected < 2000ms. Check fire-and-forget in chat.ts`);
    });
  }

  await test('Agent reply appeared on Telegram', () => {
    assert(agentMsgFound, 'Agent Lee reply message not found on Telegram within 30s');
  });
